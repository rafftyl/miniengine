...
void KeyMover::onKeyHold(sf::Keyboard::Key key, const mini::ModifierKeys& modifiers)
{
	...
	{
	case sf::Keyboard::D:
		owner.move(sf::Vector2f(500, 0) * mini::Time::getDeltaTime());
		break;
	case sf::Keyboard::A:
		owner.move(sf::Vector2f(-500, 0) * mini::Time::getDeltaTime());
		break;
	case sf::Keyboard::W:
		owner.move(sf::Vector2f(0, -500) * mini::Time::getDeltaTime());
		break;
	case sf::Keyboard::S:
		owner.move(sf::Vector2f(0, 500) * mini::Time::getDeltaTime());
		break;
...
void KeyMover::onMouseWheel(float delta)
{
	...
	cam->setOrthoSize(orthoSize + sf::Vector2f(30 * delta * aspect, 30 * delta));
}
